...
def server_train(model, optimizer, trainset, testset, epochs, args, isTest=True):
    ...
    for epoch in range(epochs):
        ...
        for _,data in enumerate(dataloader):
            ...
            if args.model == 'VAE' or args.model == 'BetaVAE':
                ...
                loss, recon_loss, reg_loss = model.loss(inputs, recon, mu, logvar)
            elif args.model == 'WAE':
                recon, z = model(inputs)
                loss, recon_loss, reg_loss = model.loss(inputs, recon, z)
            else:
        ...
        if isTest and (epoch+1)%1==0:
            ...
            serlogger.info(f'E{epoch} / loss:{loss_rec:.3f} / recon_loss:{recon_loss_rec:.3f} / reg_loss:{reg_loss_rec:.3f}')

            dis, label, normal_loss, loss = choose_test(model, testset, args.model)
            auc_cur = auc_value(label, dis)
            serlogger.info(f'E{epoch} / auc:{auc_cur:.3f} / norm_loss:{normal_loss:.3f} / loss:{loss:.3f}')
...
def DSVDD_train(model, optimizer, trainset, testset, epochs, args, isTest=True):
    batch_size = args.batch_size
    ...
    for epoch in range(epochs):
        ...
        if isTest and (epoch+1)%1==0:
            loss_rec = loss_rec/batch_num
            serlogger.info(f'E{epoch} / loss:{loss_rec}')

            _, dis, label, normal_loss, loss = DSVDD_test(model, testset, fixed_center)
            # sorted_score = find_DSVDD_split(model, trainset, fixed_center, split_rate=split_rate)
            # auc_cur, f1_cur, recall_cur, precision_cur = metrics_value(label, dis, sorted_score)
            auc_cur = auc_value(label, dis)
            serlogger.info(f'E{epoch} / auc:{auc_cur:.3f} / norm_loss:{normal_loss} / loss:{loss}')
            print(f'E{epoch} / auc:{auc_cur:.3f} / norm_loss:{normal_loss} / loss:{loss}')

            model_params = model.state_dict()
            model_params['center'] = fixed_center
            torch.save(model_params, f'./checkpoint/{args.dataset}-{args.anomaly_rate}-{args.model_size}-{args.cmp_model}-{epoch}.pth')
...
def DAGMM_train(model, optimizer, trainset, testset, epochs, args, isTest=True):
    batch_size = args.batch_size
    ...
    for epoch in range(epochs):
        ...
        if isTest and (epoch+1)%1==0:
            loss_rec = loss_rec/batch_num
            serlogger.info(f'E{epoch} / loss:{loss_rec:.3f}')

            _, Energy, label, normal_loss, loss = DAGMM_test(model, testset)
            # sorted_score = find_DAGMM_split(model, trainset, split_rate=split_rate)
            # auc_cur, f1_cur, recall_cur, precision_cur = metrics_value(label, Energy, sorted_score)
            auc_cur = auc_value(label, Energy)
            serlogger.info(f'E{epoch} / auc:{auc_cur:.3f} / norm_loss:{normal_loss:.3f} / loss:{loss:.3f}')
            print(f'E{epoch} / auc:{auc_cur:.3f} / norm_loss:{normal_loss:.3f} / loss:{loss:.3f}')

            torch.save(model.state_dict(), f'./checkpoint/{args.dataset}-{args.anomaly_rate}-{args.model_size}-{args.cmp_model}-{epoch}.pth')
